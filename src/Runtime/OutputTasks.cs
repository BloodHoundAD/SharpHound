...
namespace SharpHound.Core.Behavior
{
    public class OutputTasks
    {
        ...
        public static readonly ConcurrentDictionary<string, int> ComputerStatusCount = new ConcurrentDictionary<string, int>();
        ...
        {
            PrintStatus();
            _statusTimer = new Timer(context.StatusInterval);
        ...
        public static async Task CompleteOutput(Context context)
        {
            ...
            if (context.ZipFilename != null)
                finalName = ClientHelpers.ResolveFileName(context, context.ZipFilename, "zip", true);
            else
            {
                finalName = ClientHelpers.ResolveFileName(context, "BloodHound", "zip", true);
            }
        ...
        public static void AddComputerStatus(ComputerStatus status)
        {
            ...
            var hash = $"{status.GetHashCode()}-{Regex.Replace(status.ToString(), @"\t|\n|\r", "")}"; // TODO: How should this be calculated.
